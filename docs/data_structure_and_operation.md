...
moka已经保证了每个单key操作是原子的。读读之间显然是不冲突的，写写之间是串行的，因此也不会产生原子性问题。
...
单个key的读取一定是串行的。但是当多key读遇到了多次的单key的写。比如这里执行mget读k1和k2：读取k1->写入k1->写入k2->读取k2。导致原子性出现问题，多key写同理。因此在这里对所有写操作和多key读的操作，都加上一把锁来保证各自的原子性。



通信模式上，理论上最好是通过request_id的模式来实现multiplexing（多路复用）。但是为了兼容redis的协议，推荐客户端用更大的连接数来避免raft更严重的队头阻塞问题。