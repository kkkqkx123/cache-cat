...
Rust的moka库已经保证了每个单key操作是原子的。读读之间显然是不冲突的，写写之间是串行的，因此也不会产生原子性问题。
...
1. 单key读上读锁，多key读上写锁。单key写上读锁，多key写上写锁。
2. 读统一上读锁，写统一上写锁。

但是考虑到redis中单key的读写是绝对占主导的。为了让读写并发，这里优先选择方案1。然而方案1不能批量加锁，几百条写指令的情况下无法进行锁粗化。
